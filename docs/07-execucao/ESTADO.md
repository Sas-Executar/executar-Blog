...
- **O que foi feito:**
  ...
  - Continuação, mesmo dia: `/guia-de-estilo/` e `DashboardSinteseTdah` — deixados de fora da primeira entrega — foram reescritos no mesmo vocabulário. Os componentes `ui/Botao`, `ui/Campo`, `ui/Chip`, `ui/Marca`, `ui/Rotulo` e `ui/Selo` (nada além do guia os usava) foram apagados; o guia passou a demonstrar os componentes reais (`.edit-btn`, `.acao`, `.campo-busca`, `.segmentado`, `.filtro-chip`, `.categoria-chip`, `.rc-card`, `.lista-simples`, `.tags`, a biblioteca de callouts, o bloco de decisão). `.categoria-chip` subiu de um `<style>` escopado em `Hoje.astro` para `integracao.css` (reutilizável). O painel TDAH manteve o Chart.js, só trocando os utilitários Tailwind pelas classes e tokens da referência.
- **Bugs corrigidos durante a validação** (detalhe no ADR-019): overflow horizontal de 344px em toda página no celular (regra de alvo de toque sem media query vencendo o `display:none` da própria referência pela ordem da folha); item de grid maior que a viewport na Home e nas grades do guia de estilo (SVG/formulário/segmentado sem `min-width:0`, e um mosaico de 6 colunas do mood board mal preenchido); alvo de toque de 32px no botão "copiar" do Expressive Code (regra genérica perdida ao enxugar `tokens.css`); contraste insuficiente em modo escuro em seis componentes (`.filtro-chip`, `.contagem`, `.estado-busca`, `.resultados mark`, a marca sobre fundo escuro do guia, o item ativo da barra inferior simulada) e no botão primário/na amostra "Azul" do guia em modo claro e escuro (a referência zera `a{color:inherit}` fora de camada e vence os utilitários do Tailwind, dentro de `@layer components`, na cascata de camadas; e alguns tokens fixos como `--brand-dark` não acompanham a troca de tema, então uma superfície que troca de cor por tema ao lado de um texto fixo pode perder contraste); tema do Expressive Code invertido (`themes:['github-dark','github-light']`, pré-existente, corrigido por ser causa raiz simples); texto do artigo não crescia com "Texto maior" (a referência fixa `.article-text` em `px`, não `rem`).
- **Limitação conhecida, reportada e não corrigida:** alguns tokens de sintaxe do Expressive Code no exemplo `npm run check`/`Plain text` do guia de estilo (nome de função em modo claro, comentário em modo escuro) usam cores dos temas `github-light`/`github-dark` do Shiki abaixo de 4,5:1 contra a paleta da referência. Só essas palavras, nesse exemplo, nessa página; não afeta blocos de código dos artigos reais (que não passam mais pelo Expressive Code).
