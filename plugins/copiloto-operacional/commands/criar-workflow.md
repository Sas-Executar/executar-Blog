---
description: "Propõe um workflow (ops/workflows) por PR — o merge humano aprova"
argument-hint: "<area> + definição YAML (ou caminho de arquivo)"
---

Chame a ferramenta `executar` do servidor MCP `copiloto` (plugin copiloto-operacional) com `linha` = `/criar-workflow $ARGUMENTS`.
...
Envie a definição YAML exatamente como o usuário deu, em `payload` (ou o caminho em `arquivo`). Workflow novo nasce com `estado: PROPOSTO`.
