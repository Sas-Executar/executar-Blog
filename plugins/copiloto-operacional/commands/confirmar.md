---
description: "Executa um plano pendente com o token de confirmação (uso único, 30 min)"
argument-hint: "<token>"
---

Chame a ferramenta `executar` do servidor MCP `copiloto` (plugin copiloto-operacional) com `linha` = `/confirmar $ARGUMENTS`.
...
Só use um token que o próprio usuário digitou neste comando.
