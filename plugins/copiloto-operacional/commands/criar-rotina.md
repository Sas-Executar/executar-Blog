---
description: "Propõe uma rotina (ops/routines) por PR — o merge humano aprova"
argument-hint: "+ definição YAML com id: ROT-NNN (ou caminho de arquivo)"
---

Chame a ferramenta `executar` do servidor MCP `copiloto` (plugin copiloto-operacional) com `linha` = `/criar-rotina $ARGUMENTS`.
...
Envie o YAML exatamente como veio, em `payload` (ou `arquivo`).
